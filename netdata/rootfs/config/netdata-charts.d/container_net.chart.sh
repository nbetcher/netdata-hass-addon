...
cn_define_chart() {
    id="$1"
    iface="$2"
    echo "CHART container_net_${id} container_net_${id} 'Container Net ${iface}' kilobits/s network container.net area"
...
container_net_create() {
    ...
    DATA="$(awk '
        ...
        NR>2 {
            ...
            if (iface=="lo") next
            id=sanitize(iface)
            printf("CHART container_net_%s container_net_%s '\''Container Net %s'\'' kilobits/s network container.net area\n", id, id, iface)
